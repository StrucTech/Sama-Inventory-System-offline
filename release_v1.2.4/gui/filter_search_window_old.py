...
class FilterSearchWindow:
    ...
    def on_filter_change(self, filter_type="unknown"):
        """معالج تغيير الفلاتر مع تتبع مفصل"""
        print(f"🔄 تغيير فلتر: {filter_type}")
        
        # تطبيق فوري للقوائم المنسدلة
        self.apply_filters()
        
        # تحديث عنوان النافذة لإظهار التفاعل
        current_title = self.window.title()
        if "🔄" not in current_title:
            self.window.title("🔄 " + current_title)
            # إعادة العنوان الأصلي بعد ثانية
            self.window.after(1000, lambda: self.window.title(current_title.replace("🔄 ", "")))
    ...
    def delayed_filter(self):
        ...
        print("⏱️ تطبيق مؤجل للفلاتر...")
        # تطبيق الفلترة بعد 1000 مللي ثانية (ثانية واحدة)
        self._filter_timer = self.window.after(1000, self.apply_filters)
    
    def flash_title(self, message):
        """إضاءة العنوان برسالة مؤقتة"""
        original_title = "🔍 البحث باستخدام الفلاتر"
        if self.current_user and self.current_user.get('user_type') == 'user':
            project_id = self.current_user.get('project_id', '')
            if project_id:
                original_title += f" - مشروع {project_id}"
        
        # تغيير العنوان مؤقتاً
        self.window.title(f"{message} | {original_title}")
        
        # إعادة العنوان الأصلي بعد 2 ثانية
    ...
    def setup_filters_frame(self, parent):
        ...
        self.date_to_entry.bind("<FocusOut>", lambda e: self.apply_filters())
        ttk.Button(date_frame, text="📅", width=3,
        ...
        self.item_combobox.bind("<<ComboboxSelected>>", lambda e: self.on_filter_change("item"))
        ...
        self.category_combobox.bind("<<ComboboxSelected>>", lambda e: self.on_filter_change("category"))
        ...
        self.project_combobox.bind("<<ComboboxSelected>>", lambda e: self.on_filter_change("project"))
    ...
    def apply_filters(self):
        ...
        try:
            ...
            # مسح النتائج السابقة
            previous_count = len(self.results_tree.get_children())
            for item in self.results_tree.get_children():
                self.results_tree.delete(item)
            
            print(f"🗑️ تم مسح {previous_count} نتيجة سابقة")
            self.filtered_data = []
            ...
            print(f"➕ تم إضافة {added_count} نتيجة للجدول")
            ...
            if len(self.filtered_data) == 0:
                ...
                print("💡 تم إضافة رسالة 'لا توجد نتائج'")
            
            # تحديث الإحصائيات
            self.update_statistics(current_stock_count, activity_count)
            
            # إضافة إشارة بصرية لتأكيد التطبيق
            self.flash_title("✅ تم تطبيق الفلاتر")
    ...
    def matches_filters(self, item_name, category, project, date, 
        ...
        if filter_item and filter_item != "جميع العناصر":
            if not item_name:
                return False
            # التحقق من وجود النص في اسم العنصر (بحث جزئي)
        ...
        if filter_category and filter_category != "جميع التصنيفات":
            if not category:
                return False
            # التحقق من وجود النص في التصنيف (بحث جزئي)
        ...
        if filter_project and filter_project != "جميع المشاريع":
            if not project:
                return False
            # التحقق من وجود النص في المشروع (بحث جزئي)
