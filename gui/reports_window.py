...
class ReportsWindow:
    ...
    def load_data(self):
        ...
        try:
            ...
            # التحقق من نوع المستخدم ومشروعه
            ...
            if hasattr(self.sheets_manager, 'get_all_items_raw'):
                all_items = self.sheets_manager.get_all_items_raw()
                
                # فلترة البيانات للمستخدمين العاديين
                if is_regular_user and user_project_id:
                    self.all_items = []
                    for item in all_items:
                        if len(item) >= 4:  # التأكد من وجود عمود المشروع
                            item_project = item[3] if item[3] else ""
                            if item_project == user_project_id:
                                self.all_items.append(item)
                    print(f"📦 تم تحميل {len(self.all_items)} عنصر من مشروع {user_project_id} (من أصل {len(all_items)})")
                else:
            ...
            if hasattr(self.sheets_manager, 'get_activity_log'):
                all_activity = self.sheets_manager.get_activity_log()
                
                # فلترة سجل النشاط للمستخدمين العاديين
                if is_regular_user and user_project_id:
                    self.activity_log = []
                    for log in all_activity:
                        if len(log) >= 6:  # التحقق من وجود التفاصيل
                            log_details = log[5] if len(log) > 5 else ""
                            # البحث عن رقم المشروع في التفاصيل
                            if user_project_id in log_details or not log_details:
                                self.activity_log.append(log)
                    print(f"📋 تم تحميل {len(self.activity_log)} إدخال من سجل النشاط لمشروع {user_project_id} (من أصل {len(all_activity)})")
                else:
                    self.activity_log = all_activity
...
def show_reports_window(parent, sheets_manager, current_user=None):
    """دالة مساعدة لعرض نافذة التقارير"""
    return ReportsWindow(parent, sheets_manager, current_user)