...
# -*- coding: utf-8 -*-
"""
نافذة البحث باستخدام الفلاتر - إصدار مُصحح ومُحسن
"""
...
class FilterSearchWindow:
    ...
    def setup_filters_frame(self, parent):
        ...
        # فلتر المشروع (مخفي للمستخدم العادي إذا كان له مشروع محدد)
    ...
    def setup_stats_frame(self, parent):
        ...
        stats_frame = ttk.LabelFrame(parent, text="📊 الإحصائيات (مجاميع الكميات)", padding="10")
        ...
        # إجمالي الكمية الحالية
        ttk.Label(left_stats, text="إجمالي الكمية الحالية", font=("Arial", 12, "bold")).pack()
        self.total_quantity_label = ttk.Label(left_stats, text="0", font=("Arial", 14), foreground="blue")
        self.total_quantity_label.pack()
        
        # عدد العناصر
        ttk.Label(center_left_stats, text="عدد العناصر", font=("Arial", 12, "bold")).pack()
        self.item_count_label = ttk.Label(center_left_stats, text="0", font=("Arial", 14), foreground="green")
        self.item_count_label.pack()
        
        # عدد التصنيفات
        ttk.Label(center_right_stats, text="عدد التصنيفات", font=("Arial", 12, "bold")).pack()
        self.category_count_label = ttk.Label(center_right_stats, text="0", font=("Arial", 14), foreground="orange")
        self.category_count_label.pack()
    ...
    def show_date_picker(self, date_type):
        ...
        if current_from or current_to:
            ...
            ttk.Label(frame, text=range_text, font=("Arial", 9), foreground="blue").pack(pady=(0, 5))
        ...
        try:
            ...
            if self.current_user and self.current_user.get('user_type') == 'user':
                ...
                if user_project:
                    ...
                    print(f"🔒 تم فلترة البيانات للمستخدم: {len(self.all_items)} عنصر لمشروع {user_project}")
    ...
    def update_filter_options(self):
        ...
        # تحديث قائمة المشاريع فقط للمدير
    ...
    def apply_filters(self):
        ...
        try:
            ...
            # للمستخدم العادي، فرض مشروعه
            if self.current_user and self.current_user.get('user_type') == 'user':
                user_project = self.current_user.get('project_id', '')
                if user_project:
                    filter_project = user_project
            
            print(f"🔍 تطبيق الفلاتر: العنصر='{filter_item}', التصنيف='{filter_category}', المشروع='{filter_project}'")
            if filter_date_from or filter_date_to:
                print(f"📅 نطاق التاريخ: من {filter_date_from} إلى {filter_date_to}")
            ...
            if len(self.filtered_data) == 0:
                self.results_tree.insert("", "end", values=(
                    ...
                    "للبحث", 
                    "", 
                    "لا توجد عناصر تطابق الفلاتر المحددة"
                ))
            
            # تحديث الإحصائيات
            self.update_statistics()
            
            print(f"✅ تم فلترة {len(self.filtered_data)} نتيجة")
    ...
    def matches_filters(self, item_name, category, project, date, 
        ...
        # فلتر التصنيف - يدعم البحث الجزئي  
        if filter_category and filter_category != "جميع التصنيفات":
            if not category:
                return False
            if filter_category.lower().strip() not in category.lower().strip():
                return False
        
        # فلتر المشروع - يدعم البحث الجزئي
        if filter_project and filter_project != "جميع المشاريع":
            if not project:
                return False
            if filter_project.lower().strip() not in project.lower().strip():
                return False
    ...
    def update_statistics(self):
        ...
        # تحديث التسميات
        self.total_quantity_label.config(text=f"{total_quantity:,.1f}")
        self.item_count_label.config(text=str(item_count))
        self.category_count_label.config(text=str(len(categories)))
        self.projects_count_label.config(text=str(len(projects)))
        
        print(f"📊 الإحصائيات: كمية={total_quantity:,.1f}, عناصر={item_count}, تصنيفات={len(categories)}, مشاريع={len(projects)}")
