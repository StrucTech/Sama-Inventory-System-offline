...
class MainWindow:
    ...
    def setup_ui(self):
        ...
        self.reports_btn = tk.Button(buttons_frame, text="🔍 بحث باستخدام فلاتر", 
                                    ...
                                    bg=LUXURY_MAIN_COLORS['bg_card'], 
                                    fg=LUXURY_MAIN_COLORS['text_primary'],
                                    activebackground=LUXURY_MAIN_COLORS['bg_hover'],
                                    **luxury_button_config)
    ...
    def edit_recent_addition(self):
        ...
        try:
            ...
            if not enhanced_manager.connect():
                messagebox.showerror("خطأ", "فشل في الاتصال لتحليل العمليات الحديثة")
                return
    ...
    def get_user_recent_additions(self, enhanced_manager, item_name, username):
        ...
        all_activities = enhanced_manager.get_activity_log_new_format()
        ...
        for activity in all_activities:
            if len(activity) >= 12:  # التأكد من وجود جميع الأعمدة
                date_str = activity[0]  # التاريخ
                time_str = activity[1]  # الوقت  
                operation_type = activity[2]  # نوع العملية
                activity_item = activity[3]  # العنصر
                category = activity[4]  # التصنيف
                quantity_added = activity[5]  # الكمية المضافة
                quantity_removed = activity[6]  # الكمية المسحوبة
                previous_quantity = activity[7]  # الكمية السابقة
                current_quantity = activity[8]  # الكمية الحالية
                recipient_name = activity[9]  # اسم المستلم/المستخدم
                project_number = activity[10]  # رقم المشروع
                details = activity[11] if len(activity) > 11 else ""  # التفاصيل
    ...
    def show_recent_addition_dialog(self, selected_item, recent_additions):
        ...
        tk.Label(info_frame, text="📌 يمكنك تعديل فقط الكمية التي أضفتها خلال آخر 24 ساعة", 
                font=("Arial", 8), fg="blue", wraplength=350).pack()
        tk.Label(info_frame, text="📌 الحد الأقصى: " + str(int(total_recent_added)), 
                font=("Arial", 8), fg="red").pack()
        ...
        def on_confirm():
            try:
                ...
                # تحقق إضافي من صحة القيمة
                if new_quantity == total_recent_added:
                    print("⚠️ تحذير: القيمة الجديدة مطابقة للقديمة!")
                    if not messagebox.askyesno("تأكيد", 
                        f"القيمة الجديدة ({new_quantity}) مطابقة للقيمة القديمة ({total_recent_added}).\n"
                ...
                if new_quantity < 0:
                    ...
                    return
                if new_quantity > total_recent_added:
                    messagebox.showerror("خطأ", f"لا يمكن تعديل أكثر من {total_recent_added} (الكمية المضافة خلال آخر 24 ساعة)")
                    return
    ...
    def process_recent_addition_edit(self, selected_item, old_added_quantity, new_added_quantity):
        ...
        try:
            ...
            if abs(old_added_quantity - new_added_quantity) < 0.001:  # تقريباً نفس القيمة
                print("DEBUG: لا يوجد تغيير في الكمية - إلغاء العملية")
                messagebox.showinfo("تنبيه", "لم يتم إجراء أي تغيير في الكمية")
                return
            
            # حساب الكمية الأصلية قبل الإضافة
            original_quantity_before_addition = current_quantity - old_added_quantity
            
            # حساب الكمية الجديدة بعد التعديل
            new_total_quantity = original_quantity_before_addition + new_added_quantity
            ...
            print(f"  - الكمية الأصلية: {original_quantity_before_addition}")
            print(f"  - الكمية الجديدة الإجمالية: {new_total_quantity}")
            
            if new_total_quantity < 0:
                messagebox.showerror("خطأ", "لا يمكن أن تكون الكمية الإجمالية سالبة")
                return
            
            if original_quantity_before_addition < 0:
                messagebox.showerror("خطأ", "خطأ في الحساب - الكمية الأصلية لا يمكن أن تكون سالبة")
                return
            ...
            def update_quantity():
                try:
                    ...
                    if self.sheets_manager.update_quantity(selected_item["row"], new_total_quantity):
                        # تسجيل العملية
                        quantity_change = new_added_quantity - old_added_quantity
                        details = f"تعديل كمية مضافة حديثة من {old_added_quantity} إلى {new_added_quantity} (تغيير إجمالي: {quantity_change:+})"
                        self.sheets_manager._log_activity(
                            "تعديل كمية حديثة", 
                            selected_item['item_name'], 
                            str(new_total_quantity), 
                            self.current_user.get('username', ''),  # recipient_name
                            details,  # details
                            selected_item.get('category', 'متنوع'),  # category
                            self.current_user.get('username', '')  # current_user
                        )
    ...
    def open_analytics_window(self):
        """فتح نافذة التحليل والرؤى للمدراء فقط."""
        if not self.current_user or self.current_user.get('user_type') != 'admin':
            messagebox.showerror("خطأ", "هذه الميزة متاحة للمدراء فقط!")
            return
        ...
        try:
            ...
            if not enhanced_manager.connect():
                messagebox.showerror("خطأ", "فشل في الاتصال بـ Google Sheets للتحليل!")
                return
            
            # فتح نافذة التحليل
            from analytics_window import AnalyticsWindow
            analytics_window = AnalyticsWindow(self.root, enhanced_manager, self.current_user)
            print("✅ تم فتح نافذة التحليل والرؤى")
            
        except ImportError as e:
            print(f"⚠️ لم يتم العثور على نافذة التحليل: {e}")
            messagebox.showinfo("معلومات", "نافذة التحليل والرؤى قيد التطوير...")
        except Exception as e:
            print(f"❌ خطأ في فتح نافذة التحليل: {e}")
            messagebox.showerror("خطأ", f"فشل في فتح نافذة التحليل:\n{e}")
    ...
    def open_filter_search_window(self):
        ...
        try:
            ...
            # الاتصال والتحقق من الشيت الجديد
            if enhanced_manager.connect():
                print("✅ تم الاتصال بـ Google Sheets")
                
                # التحقق من وجود الشيت الجديد
                activity_data = enhanced_manager.get_activity_log_new_format()
                if activity_data:
                    ...
                    messagebox.showwarning("تحذير", 
                        "لا توجد بيانات في الشيت الجديد. يرجى التأكد من وجود البيانات أو استخدام الميزة القديمة.")
            else:
